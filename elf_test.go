...
// TestELFSearchROPGadgets tests searching for ROP gadgets by regex
func TestELFSearchROPGadgets(t *testing.T) {
    t.Logf("Testing ROP gadget search (%s)...", elfFile)
    e, _ := NewELF(elfFile)
    gadgets, err := e.SearchROPGadgets(".*")
    if err != nil {
        t.Fatal(err)
    }
    ...
    gadgets, err = e.SearchROPGadgets("add rsp, 8 ; pop rbx ; pop rbp ; pop r12 ; pop r13 ; pop r14 ; pop r15 ; ret")
    if err != nil {
        t.Fatal(err)
    }
